...
-- Arroyo Don Juan
('Arroyo Don Juan', (SELECT id FROM geo.zones WHERE name = 'Centro'),
...
-- Arroyo de la 84
('Arroyo de la 84', (SELECT id FROM geo.zones WHERE name = 'Riomar'),
...
-- Arroyo del Country
('Arroyo del Country', (SELECT id FROM geo.zones WHERE name = 'Norte Centro Histórico'),
...
-- Arroyo de la 41
('Arroyo de la 41', (SELECT id FROM geo.zones WHERE name = 'Soledad'),
...
-- Shopping centers
('Centro Comercial Buenavista', 'mall', ST_GeomFromText('POINT(-74.7950 11.0050)', 4326), (SELECT id FROM geo.zones WHERE name = 'Riomar'), 'Calle 98 con Carrera 53', 5000, '{"parking_spots": 1200, "floors": 3}'),
('Centro Comercial Único', 'mall', ST_GeomFromText('POINT(-74.7850 10.9920)', 4326), (SELECT id FROM geo.zones WHERE name = 'Norte Centro Histórico'), 'Calle 82 con Carrera 53', 4000, '{"parking_spots": 800}'),

-- Hospitals
('Clínica Portoazul', 'hospital', ST_GeomFromText('POINT(-74.7890 11.0100)', 4326), (SELECT id FROM geo.zones WHERE name = 'Riomar'), 'Carrera 53 No. 98-56', 300, '{"emergency_room": true, "icu_beds": 40}'),
('Hospital Universidad del Norte', 'hospital', ST_GeomFromText('POINT(-74.7820 11.0180)', 4326), (SELECT id FROM geo.zones WHERE name = 'Riomar'), 'Km 5 Vía Puerto Colombia', 400, '{"trauma_center": true}'),

-- Stadiums
('Estadio Metropolitano Roberto Meléndez', 'stadium', ST_GeomFromText('POINT(-74.8080 10.9520)', 4326), (SELECT id FROM geo.zones WHERE name = 'Metropolitana'), 'Calle 72 con Circunvalar', 46788, '{"events_cause_traffic": true, "major_venue": true}'),

-- Universities
('Universidad del Norte', 'school', ST_GeomFromText('POINT(-74.7810 11.0190)', 4326), (SELECT id FROM geo.zones WHERE name = 'Riomar'), 'Km 5 Vía Puerto Colombia', 18000, '{"students": 18000, "peak_hours": "7-9am, 5-7pm"}'),
('Universidad del Atlántico', 'school', ST_GeomFromText('POINT(-74.7920 10.9850)', 4326), (SELECT id FROM geo.zones WHERE name = 'Norte Centro Histórico'), 'Carrera 30', 25000, '{"students": 25000, "public_university": true}'),

-- Transport hubs
('Terminal de Transportes', 'transport_hub', ST_GeomFromText('POINT(-74.8070 10.9450)', 4326), (SELECT id FROM geo.zones WHERE name = 'Metropolitana'), 'Carrera 14 con Calle 54', 3000, '{"intercity_buses": true, "daily_passengers": 15000}'),
('Aeropuerto Ernesto Cortissoz', 'transport_hub', ST_GeomFromText('POINT(-74.7810 10.8900)', 4326), (SELECT id FROM geo.zones WHERE name = 'Soledad'), 'Vía Soledad', 8000, '{"international": true, "daily_flights": 80}');
...
('Estación Virtual Centro', 'virtual', ST_GeomFromText('POINT(-74.7800 10.9800)', 4326), (SELECT id FROM geo.zones WHERE name = 'Centro'), 5, '{"data_source": "openweather"}'),
('Estación Virtual Norte', 'virtual', ST_GeomFromText('POINT(-74.7850 11.0100)', 4326), (SELECT id FROM geo.zones WHERE name = 'Riomar'), 8, '{"data_source": "openweather"}'),
('Estación Virtual Sur', 'virtual', ST_GeomFromText('POINT(-74.7950 10.9400)', 4326), (SELECT id FROM geo.zones WHERE name = 'Metropolitana'), 3, '{"data_source": "openweather"}'),
('Estación Real Aeropuerto', 'real', ST_GeomFromText('POINT(-74.7810 10.8895)', 4326), (SELECT id FROM geo.zones WHERE name = 'Soledad'), 12, '{"data_source": "IDEAM", "station_code": "13015020"}');
